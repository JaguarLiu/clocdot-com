...
fastify.post('/api/admin/departments', { preHandler: fastify.requireModule('settings'), schema: { body: S.department } }, async (request, reply) => {
  ...
  const d = await fastify.prisma.$transaction(async (tx) => {
    ...
    await writeAudit(tx, auditContext(request), {
      action: 'department.created', entityType: 'department', entityId: created.id,
      after: { name: created.name, parentId: created.parentId, managerId: created.managerId },
    })
    return created
  })
...
fastify.patch('/api/admin/departments/:id', { preHandler: fastify.requireModule('settings'), schema: { body: S.department } }, async (request, reply) => {
  ...
  const target = await assertOwnedByCompany(
    (did) => fastify.prisma.department.findUnique({
      where: { id: did }, select: { id: true, companyId: true, name: true, parentId: true, managerId: true },
    }),
  ...
  const [d] = await fastify.prisma.$transaction([
    fastify.prisma.department.update({ where: { id }, data, include: DEPT_INCLUDE }),
    ...(change ? [writeAudit(fastify.prisma, auditContext(request), {
      action: 'department.updated', entityType: 'department', entityId: id, ...change,
    })] : []),
  ])
  return serializeDept(d)
})
...
fastify.delete('/api/admin/departments/:id', { preHandler: fastify.requireModule('settings') }, async (request, reply) => {
  ...
  await fastify.prisma.$transaction([
    ...
    writeAudit(fastify.prisma, auditContext(request), {
      action: 'department.deleted', entityType: 'department', entityId: id,
      before: { name: target.name, parentId: target.parentId, managerId: target.managerId },
    }),
  ])
  return { ok: true }
})
...
fastify.post('/api/admin/departments/:deptId/roles', { preHandler: fastify.requireModule('settings'), schema: { body: S.roleCreate } }, async (request, reply) => {
  ...
  try {
    const r = await fastify.prisma.$transaction(async (tx) => {
      ...
      await writeAudit(tx, auditContext(request), {
        action: 'role.created', entityType: 'role', entityId: created.id,
        after: { name, permissions, departmentId: deptId },
      })
      return created
    })
...
fastify.patch('/api/admin/roles/:id', { preHandler: fastify.requireModule('settings'), schema: { body: S.rolePatch } }, async (request, reply) => {
  ...
  const role = await assertOwnedByCompany(
    (rid) => fastify.prisma.role.findUnique({
      where: { id: rid }, select: { id: true, companyId: true, isAdmin: true, name: true, permissions: true },
    }),
  ...
  try {
    const change = diffFields(role, data)
    const [r] = await fastify.prisma.$transaction([
      fastify.prisma.role.update({ where: { id }, data }),
      ...(change ? [writeAudit(fastify.prisma, auditContext(request), {
        action: 'role.updated', entityType: 'role', entityId: id, ...change,
      })] : []),
    ])
...
fastify.delete('/api/admin/roles/:id', { preHandler: fastify.requireModule('settings') }, async (request, reply) => {
  ...
  const role = await assertOwnedByCompany(
    (rid) => fastify.prisma.role.findUnique({
      where: { id: rid },
      select: { id: true, companyId: true, isAdmin: true, name: true, permissions: true, departmentId: true },
    }),
    id, request.companyId, reply, (r) => r.companyId,
  )
  if (!role) return
  ...
  await fastify.prisma.$transaction([
    fastify.prisma.role.delete({ where: { id } }),
    writeAudit(fastify.prisma, auditContext(request), {
      action: 'role.deleted', entityType: 'role', entityId: id,
      before: { name: role.name, permissions: role.permissions, departmentId: role.departmentId },
    }),
  ])
  return { ok: true }
})
...
fastify.delete('/api/admin/users/:id', { preHandler: fastify.requireModule('employees') }, async (request, reply) => {
  ...
  await fastify.prisma.$transaction([
    ...
    writeAudit(fastify.prisma, auditContext(request), {
      action: 'user.deleted', entityType: 'user', entityId: id, targetUserId: id,
    }),
  ])
