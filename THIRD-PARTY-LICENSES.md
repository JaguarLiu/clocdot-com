# 第三方套件授權報告

產生時間：2026-09-03T04:46:57.295Z
本專案授權：Apache-2.0
Production 相依套件數（含遞移）：195

> 本檔由 `npm run license:report` 依已安裝的 `node_modules` 產生，只涵蓋 production 相依
> （devDependencies 不隨發布散布）。發布新版本前請重新產生。
> CI 以 `npm run license:check` 把關：出現任何未查證的授權即失敗。

## 授權分佈

| 授權 | 套件數 |
|---|---:|
| MIT | 144 |
| ISC | 23 |
| Apache-2.0 | 11 |
| BlueOak-1.0.0 | 6 |
| BSD-3-Clause | 5 |
| MIT/X11 | 2 |
| Unlicense | 1 |
| UNKNOWN | 1 |
| (MIT OR GPL-3.0-or-later) | 1 |
| (MIT AND Zlib) | 1 |

## 已查證例外

| 套件 | 版本 | 授權 | 查證結論 |
|---|---|---|---|
| buffers | 0.1.1 | UNKNOWN | 未宣告授權；僅存在於建置期，不隨 admin/dist 或 Docker 映像散布 |

## 需人工確認

無。其餘 production 相依的授權皆屬已知與 Apache-2.0 散布相容的清單。

## 完整清單

| 套件 | 版本 | 授權 | 授權檔 |
|---|---|---|---|
| @babel/runtime | 7.29.2 | MIT | LICENSE |
| @fast-csv/format | 4.3.5 | MIT | LICENSE |
| @fast-csv/parse | 4.3.6 | MIT | LICENSE |
| @fastify/ajv-compiler | 4.0.5 | MIT | LICENSE |
| @fastify/cors | 11.3.0 | MIT | LICENSE |
| @fastify/error | 4.2.0 | MIT | LICENSE |
| @fastify/fast-json-stringify-compiler | 5.0.3 | MIT | LICENSE |
| @fastify/forwarded | 3.0.1 | MIT | LICENSE |
| @fastify/helmet | 13.1.0 | MIT | LICENSE |
| @fastify/jwt | 10.2.2 | MIT | LICENSE |
| @fastify/merge-json-schemas | 0.2.1 | MIT | LICENSE |
| @fastify/proxy-addr | 5.1.0 | MIT | LICENSE |
| @fastify/rate-limit | 10.3.0 | MIT | LICENSE |
| @ioredis/commands | 1.5.1 | MIT | LICENSE |
| @isaacs/cliui | 9.0.0 | BlueOak-1.0.0 | LICENSE.md |
| @lukeed/ms | 2.0.2 | MIT | license |
| @pinojs/redact | 0.4.0 | MIT | LICENSE |
| @prisma/adapter-pg | 7.8.0 | Apache-2.0 | LICENSE |
| @prisma/client | 7.8.0 | Apache-2.0 | LICENSE |
| @prisma/client-runtime-utils | 7.8.0 | Apache-2.0 | LICENSE |
| @prisma/debug | 7.8.0 | Apache-2.0 | LICENSE |
| @prisma/driver-adapter-utils | 7.8.0 | Apache-2.0 | LICENSE |
| @types/node | 25.6.0 | MIT | LICENSE |
| @types/pg | 8.20.0 | MIT | LICENSE |
| abstract-logging | 2.0.1 | MIT | — |
| ajv | 8.18.0 | MIT | LICENSE |
| ajv-formats | 3.0.1 | MIT | LICENSE |
| archiver | 5.3.2 | MIT | LICENSE |
| archiver-utils | 2.1.0 | MIT | LICENSE |
| asn1.js | 5.4.1 | MIT | LICENSE |
| async | 3.2.6 | MIT | LICENSE |
| atomic-sleep | 1.0.0 | MIT | LICENSE |
| avvio | 9.2.0 | MIT | LICENSE |
| balanced-match | 1.0.2 | MIT | LICENSE.md |
| base64-js | 1.5.1 | MIT | LICENSE |
| bcryptjs | 2.4.3 | MIT | LICENSE |
| big-integer | 1.6.52 | Unlicense | LICENSE |
| binary | 0.3.0 | MIT | — |
| bl | 4.1.0 | MIT | LICENSE.md |
| bluebird | 3.4.7 | MIT | LICENSE |
| bn.js | 4.12.5 | MIT | LICENSE |
| brace-expansion | 1.1.18 | MIT | LICENSE |
| buffer | 5.7.1 | MIT | LICENSE |
| buffer-crc32 | 0.2.13 | MIT | LICENSE |
| buffer-indexof-polyfill | 1.0.2 | MIT | LICENSE |
| buffers | 0.1.1 | UNKNOWN | — |
| chainsaw | 0.1.0 | MIT/X11 | — |
| cluster-key-slot | 1.1.2 | Apache-2.0 | LICENSE |
| compress-commons | 4.1.2 | MIT | LICENSE |
| concat-map | 0.0.1 | MIT | LICENSE |
| cookie | 1.1.1 | MIT | LICENSE |
| crc-32 | 1.2.2 | Apache-2.0 | LICENSE |
| crc32-stream | 4.0.3 | MIT | LICENSE |
| cross-spawn | 7.0.6 | MIT | LICENSE |
| dayjs | 1.11.23 | MIT | LICENSE |
| debug | 4.4.3 | MIT | LICENSE |
| denque | 2.1.0 | Apache-2.0 | LICENSE |
| dequal | 2.0.3 | MIT | license |
| duplexer2 | 0.1.4 | BSD-3-Clause | LICENSE.md |
| ecdsa-sig-formatter | 1.0.11 | Apache-2.0 | LICENSE |
| end-of-stream | 1.4.5 | MIT | LICENSE |
| exceljs | 4.4.0 | MIT | LICENSE |
| fast-csv | 4.3.6 | MIT | LICENSE |
| fast-decode-uri-component | 1.0.1 | MIT | LICENSE |
| fast-deep-equal | 3.1.3 | MIT | LICENSE |
| fast-json-stringify | 6.3.0 | MIT | LICENSE |
| fast-jwt | 6.3.2 | Apache-2.0 | LICENSE.md |
| fast-querystring | 1.1.2 | MIT | LICENSE |
| fast-uri | 3.1.7 | BSD-3-Clause | LICENSE |
| fastfall | 1.5.1 | MIT | LICENSE |
| fastify | 5.12.1 | MIT | LICENSE |
| fastify-plugin | 5.1.0 | MIT | LICENSE |
| fastparallel | 2.4.1 | ISC | LICENSE |
| fastq | 1.20.1 | ISC | LICENSE |
| fastseries | 1.7.2 | ISC | LICENSE |
| find-my-way | 9.9.0 | MIT | LICENSE |
| foreground-child | 3.3.1 | ISC | LICENSE |
| fs-constants | 1.0.0 | MIT | LICENSE |
| fstream | 1.0.12 | ISC | LICENSE |
| glob | 11.1.0 | BlueOak-1.0.0 | LICENSE.md |
| graceful-fs | 4.2.11 | ISC | LICENSE |
| helmet | 8.3.0 | MIT | LICENSE |
| html-parse-stringify | 4.0.1 | MIT | LICENSE |
| i18next | 26.3.6 | MIT | LICENSE |
| i18next-browser-languagedetector | 8.2.1 | MIT | LICENSE |
| ieee754 | 1.2.1 | BSD-3-Clause | LICENSE |
| immediate | 3.0.6 | MIT | LICENSE.txt |
| inherits | 2.0.4 | ISC | LICENSE |
| ioredis | 5.10.1 | MIT | LICENSE |
| ipaddr.js | 2.4.0 | MIT | LICENSE |
| isexe | 2.0.0 | ISC | LICENSE |
| jackspeak | 4.2.3 | BlueOak-1.0.0 | LICENSE.md |
| json-schema-ref-resolver | 3.0.0 | MIT | LICENSE |
| json-schema-traverse | 0.4.1 | MIT | LICENSE |
| jszip | 3.10.1 | (MIT OR GPL-3.0-or-later) | LICENSE.markdown |
| lazystream | 1.0.1 | MIT | LICENSE |
| lie | 3.3.0 | MIT | license.md |
| light-my-request | 6.6.0 | BSD-3-Clause | LICENSE |
| listenercount | 1.0.1 | ISC | LICENSE.md |
| lodash.defaults | 4.2.0 | MIT | LICENSE |
| lodash.difference | 4.5.0 | MIT | LICENSE |
| lodash.escaperegexp | 4.1.2 | MIT | LICENSE |
| lodash.flatten | 4.4.0 | MIT | LICENSE |
| lodash.groupby | 4.6.0 | MIT | LICENSE |
| lodash.isarguments | 3.1.0 | MIT | LICENSE |
| lodash.isboolean | 3.0.3 | MIT | LICENSE |
| lodash.isequal | 4.5.0 | MIT | LICENSE |
| lodash.isfunction | 3.0.9 | MIT | LICENSE |
| lodash.isnil | 4.0.0 | MIT | LICENSE |
| lodash.isplainobject | 4.0.6 | MIT | LICENSE |
| lodash.isundefined | 3.0.1 | MIT | LICENSE.txt |
| lodash.union | 4.6.0 | MIT | LICENSE |
| lodash.uniq | 4.5.0 | MIT | LICENSE |
| lru-cache | 5.1.1 | ISC | LICENSE |
| lucide-react | 0.574.0 | ISC | LICENSE |
| minimalistic-assert | 1.0.1 | ISC | LICENSE |
| minimatch | 3.1.5 | ISC | LICENSE |
| minimist | 1.2.8 | MIT | LICENSE |
| minipass | 7.1.3 | BlueOak-1.0.0 | LICENSE.md |
| mkdirp | 0.5.6 | MIT | LICENSE |
| mnemonist | 0.40.4 | MIT | LICENSE.txt |
| ms | 2.1.3 | MIT | license.md |
| normalize-path | 3.0.0 | MIT | LICENSE |
| obliterator | 2.0.5 | MIT | LICENSE.txt |
| on-exit-leak-free | 2.1.2 | MIT | LICENSE |
| once | 1.4.0 | ISC | LICENSE |
| package-json-from-dist | 1.0.1 | BlueOak-1.0.0 | LICENSE.md |
| pako | 1.0.11 | (MIT AND Zlib) | LICENSE |
| path-key | 3.1.1 | MIT | license |
| path-scurry | 2.0.2 | BlueOak-1.0.0 | LICENSE.md |
| pg | 8.23.0 | MIT | LICENSE |
| pg-connection-string | 2.14.0 | MIT | LICENSE |
| pg-int8 | 1.0.1 | ISC | LICENSE |
| pg-pool | 3.14.0 | MIT | LICENSE |
| pg-protocol | 1.16.0 | MIT | LICENSE |
| pg-types | 2.2.0 | MIT | — |
| pgpass | 1.0.5 | MIT | — |
| pino | 10.3.1 | MIT | LICENSE |
| pino-abstract-transport | 3.0.0 | MIT | LICENSE |
| pino-std-serializers | 7.1.0 | MIT | LICENSE |
| postgres-array | 2.0.0 | MIT | license |
| postgres-bytea | 1.0.1 | MIT | license |
| postgres-date | 1.0.7 | MIT | license |
| postgres-interval | 1.2.0 | MIT | license |
| process-warning | 5.1.0 | MIT | LICENSE |
| quick-format-unescaped | 4.0.4 | MIT | LICENSE |
| react | 19.2.8 | MIT | LICENSE |
| react-dom | 19.2.8 | MIT | LICENSE |
| react-i18next | 17.0.11 | MIT | LICENSE |
| react-router | 7.18.3 | MIT | LICENSE.md |
| react-router-dom | 7.18.3 | MIT | LICENSE.md |
| readable-stream | 3.6.2 | MIT | LICENSE |
| readdir-glob | 1.1.3 | Apache-2.0 | LICENSE |
| real-require | 0.2.0 | MIT | LICENSE.md |
| redis-errors | 1.2.0 | MIT | LICENSE |
| redis-parser | 3.0.0 | MIT | LICENSE |
| require-from-string | 2.0.2 | MIT | license |
| reusify | 1.1.0 | MIT | LICENSE |
| rfdc | 1.4.1 | MIT | LICENSE |
| rimraf | 2.7.1 | ISC | LICENSE |
| safe-buffer | 5.2.1 | MIT | LICENSE |
| safe-regex2 | 5.1.1 | MIT | LICENSE |
| safe-stable-stringify | 2.5.0 | MIT | LICENSE |
| safer-buffer | 2.1.2 | MIT | LICENSE |
| saxes | 5.0.1 | ISC | — |
| scheduler | 0.27.0 | MIT | LICENSE |
| secure-json-parse | 4.1.0 | BSD-3-Clause | LICENSE |
| semver | 6.3.1 | ISC | LICENSE |
| set-cookie-parser | 2.7.2 | MIT | LICENSE |
| setimmediate | 1.0.5 | MIT | LICENSE.txt |
| shebang-command | 2.0.0 | MIT | license |
| shebang-regex | 3.0.0 | MIT | license |
| signal-exit | 4.1.0 | ISC | LICENSE.txt |
| sonic-boom | 4.2.1 | MIT | LICENSE |
| split2 | 4.2.0 | ISC | LICENSE |
| standard-as-callback | 2.1.0 | MIT | LICENSE |
| steed | 1.1.3 | MIT | LICENSE |
| string_decoder | 1.3.0 | MIT | LICENSE |
| swr | 2.5.1 | MIT | LICENSE |
| tar-stream | 2.2.0 | MIT | LICENSE |
| thread-stream | 4.0.0 | MIT | LICENSE |
| tmp | 0.2.7 | MIT | LICENSE |
| toad-cache | 3.7.0 | MIT | LICENSE |
| traverse | 0.3.9 | MIT/X11 | LICENSE |
| undici-types | 7.19.2 | MIT | LICENSE |
| unzipper | 0.10.14 | MIT | LICENSE |
| use-sync-external-store | 1.6.0 | MIT | LICENSE |
| util-deprecate | 1.0.2 | MIT | LICENSE |
| uuid | 11.1.1 | MIT | LICENSE.md |
| which | 2.0.2 | ISC | LICENSE |
| wrappy | 1.0.2 | ISC | LICENSE |
| xmlchars | 2.2.0 | MIT | LICENSE |
| xtend | 4.0.2 | MIT | LICENSE |
| yallist | 3.1.1 | ISC | LICENSE |
| zip-stream | 4.1.1 | MIT | LICENSE |
